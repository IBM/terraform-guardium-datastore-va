...
# Azure PostgreSQL VA Config Module - Main Configuration

locals {
  # Secret names using the name_prefix for consistency
  secret_name = "${var.name_prefix}-azure-postgresql-va-password"
  ...
  storage_name = substr(replace(lower("${var.name_prefix}postgresqlva"), "/[^a-z0-9]/", ""), 0, 24)
  # Function package
  zip_file = "${path.module}/files/function.zip"
  zip_hash = filesha256(local.zip_file)
  # Construct full PostgreSQL server resource ID for firewall rules
  # Format: /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DBforPostgreSQL/flexibleServers/{serverName}
  postgresql_server_id = "/subscriptions/${data.azurerm_client_config.current.subscription_id}/resourceGroups/${var.resource_group_name}/providers/Microsoft.DBforPostgreSQL/flexibleServers/${var.postgresql_server_name}"
}
...
data "dns_a_record_set" "guardium_ip" {
  ...
  lifecycle {
    precondition {
      condition     = var.enable_public_access == false || var.guardium_hostname != ""
      error_message = "guardium_hostname must be provided when enable_public_access is true"
    }
  }
}

# Create Azure Key Vault for storing credentials
resource "azurerm_key_vault" "postgresql_credentials" {
  name                       = local.kv_name
...
resource "azurerm_key_vault_secret" "postgresql_credentials" {
  ...
  key_vault_id = azurerm_key_vault.postgresql_credentials.id

  depends_on = [azurerm_key_vault.postgresql_credentials]
}
...
resource "azurerm_service_plan" "function_plan" {
  name                = "${var.name_prefix}-postgresql-va-plan"
  location            = var.location
  resource_group_name = var.resource_group_name
  os_type             = "Linux"
...
resource "azurerm_linux_function_app" "va_config_function" {
  name                       = "${var.name_prefix}-postgresql-va-func"
  location                   = var.location
  resource_group_name        = var.resource_group_name
  ...
  app_settings = {
    ...
    "KEY_VAULT_NAME"           = azurerm_key_vault.postgresql_credentials.name
...
# 3. Invoke: curl -X POST "https://<func-name>.azurewebsites.net/api/PostgreSQLVAConfig?code=<key>" -H "Content-Type: application/json" -d '{}'
# Create firewall rule to allow Guardium server access (only if public access enabled)
resource "azurerm_postgresql_flexible_server_firewall_rule" "guardium_access" {
  count            = var.enable_public_access ? 1 : 0
  ...
  lifecycle {
    precondition {
      condition     = length(data.dns_a_record_set.guardium_ip[0].addrs) > 0
      error_message = "DNS resolution failed for guardium_hostname: ${var.guardium_hostname}"
    }
  }
}

# Create firewall rule to allow Azure services (for Function App)
# Note: 0.0.0.0 to 0.0.0.0 is Azure's special syntax for "Allow Azure Services"
...
resource "azurerm_postgresql_flexible_server_firewall_rule" "additional_rules" {
