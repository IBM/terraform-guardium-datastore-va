# AWS RDS MySQL with Vulnerability Assessment

This example demonstrates how to configure AWS RDS MySQL with Guardium Vulnerability Assessment (VA) capabilities. It sets up the necessary components to perform security assessments on your MySQL database and receive notifications about potential vulnerabilities.

## Overview

This example:

1. Configures a MySQL database for Vulnerability Assessment by creating a dedicated `sqlguard` user with appropriate permissions
2. Connects the MySQL database to Guardium Data Protection (GDP) for vulnerability scanning
...
1. **AWS Resources**:
   - An existing AWS RDS MySQL database
...
|----------|-------------|---------|
| `db_host` | MySQL database hostname or endpoint | - |
...
1. **VA Configuration**:
   - Creates a Lambda function that connects to your MySQL database
...
2. **GDP Connection**:
   - Registers the MySQL database as a data source in Guardium
...
1. **Connection Failures**:
   ...
   - Check security group rules allow traffic on port 3306
...
Note: This will not remove any existing RDS MySQL instances that were referenced but not created by this configuration.
